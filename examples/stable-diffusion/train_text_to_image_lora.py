...
from diffusers.utils import check_min_version, convert_state_dict_to_diffusers, deprecate, is_wandb_available, make_image_grid
...
def log_validation(vae, text_encoder, tokenizer, unet, args, accelerator, weight_dtype, epoch, gaudi_config):
...
# For mixed precision training we cast all non-trainable weigths (vae, non-lora text_encoder and non-lora unet) to half-precision
    ...
    # # Move text_encode, vae to gpu and cast to weight_dtype
    # text_encoder.to(accelerator.device, dtype=weight_dtype)
    # vae.to(accelerator.device, dtype=weight_dtype)
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(unet):
                ...
                if args.noise_offset:
                    ...
                    rand_device = "cpu" if latents.device.type == "hpu" else latents.device
                    noise += args.noise_offset * torch.randn(
                        (latents.shape[0], latents.shape[1], 1, 1), device=rand_device
                    )
            ...
            if accelerator.sync_gradients:
                ...
                if global_step % args.checkpointing_steps == 0:
                    if accelerator.is_main_process:
                        ...
                        GaudiStableDiffusionPipeline.save_lora_weights(
