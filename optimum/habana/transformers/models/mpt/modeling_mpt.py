...
class GaudiMptAttention(MptAttention):
    ...
    #     cache_shape = (batch_size, self.n_heads, max_seq_len, self.head_dim)
    ...
    def forward(
        ...
        """
        batch_size, seq_length = hidden_states.shape[:2]       # KOKO : [1, 256, 4096]
        mixed_qkv = self.Wqkv(hidden_states)                   # KOKO : nn.Linear
        bs, seq_len, three_times_hidden_size = mixed_qkv.shape # KOKO : [1, 256, 122880]
        mixed_qkv = mixed_qkv.view(bs, seq_len, self.n_heads * 3, self.head_dim)
        mixed_qkv = mixed_qkv.transpose(1, 2)
        query_states, key_states, value_states = (             # KOKO : shape [1, 32, 256, 128]
        ...
        # ##########################################################
        # # KOKO : start borrow from llma
        # ##########################################################
        if past_key_value is not None and not reuse_cache:
            past_key = past_key_value[0]
            past_value = past_key_value[1]
            if token_idx is not None:
                past_key.index_copy_(2, token_idx - 1, key_states)
                ...
                value_states = torch.cat([past_key_value[1], value_states], dim=2)
        #past_key_value = (key_states, value_states)
        #print(f"KOKO 1 : key_states.shape : {key_states.shape}, value_states.shape : {value_states.shape}")
        if use_cache:
            if reuse_cache:
        ...
        #key_states = gaudi_mpt_repeat_kv(key_states, self.num_key_value_groups)
        #value_states = gaudi_mpt_repeat_kv(value_states, self.num_key_value_groups)


        # ##########################################################
        # # KOKO : end
        # ##########################################################
        #print(f"KOKO 2 : key_states.shape : {key_states.shape}, value_states.shape : {value_states.shape}")
...
class GaudiMptBlock(MptBlock):
    def __init__(self, config: MptConfig):
        super().__init__(config)
        self.self_attention = GaudiMptAttention(config)
...
class GaudiMptModel(MptModel):
    ...
    def forward(
        ...
        if past_key_values[0] is not None and token_idx is None:  # because RW-cache, not standard format
            if reuse_cache: # KOKO : from llama
    ...
    def prepare_inputs_for_generation(
        ...
        **kwargs,
        ...
        if past_key_values is not None:
            ...
            # With reuse_cache, KV cache is pre allocated hence for the 1st token we can
            # slice the inputs till token idx for the fwd pass
